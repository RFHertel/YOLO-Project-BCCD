...
class Evaluator(object):
    def __init__(self, model, visiual=False):
        self.classes = ['RBC', 'WBC', 'Platelets']#cfg.DATA["CLASSES"]  # ['RBC', 'WBC', 'Platelets']
        print(f"DEBUG: Evaluator initialized with classes: {self.classes}")  # Add this
        self.pred_result_path = os.path.join(r"C:\AWrk\YOLO_Project_BCCD\yolov3_pytorch", 'data_bccd', 'results')
        self.val_data_path = r"C:\AWrk\YOLO_Project_BCCD\yolov3_pytorch\data_bccd\BCCD" #cfg.DATA_PATH  # Points to BCCD data
    ...
    def APs_voc(self, multi_test=False, flip_test=False):
        ...
        for img_ind in tqdm(img_inds):
            ...
            for bbox in bboxes_prd:
                ...
                # Make sure class_ind is within range for BCCD (0-2)
                if class_ind >= len(self.classes):
                    continue
    ...
    def __calc_APs(self, iou_thresh=0.5, use_07_metric=False):
        """
        Calculate AP for each BCCD class
        """
        print(f"DEBUG: About to evaluate classes: {self.classes}")  # Add this
        filename = os.path.join(self.pred_result_path, 'comp4_det_test_{:s}.txt')
        cachedir = os.path.join(self.pred_result_path, 'cache')
        annopath = os.path.join(self.val_data_path, 'Annotations', '{}.xml')
        imagesetfile = os.path.join(self.val_data_path, 'ImageSets', 'Main', 'test.txt')

        print(f"Looking for annotations at: {annopath}")
        print(f"Classes to evaluate: {self.classes}")
