# # scripts/evaluate_model_enhanced.py
# import json
...
#         # Calculate both micro and macro averaged metrics
...
#                     'support': class_counts.get(class_name, 0),  # Number of instances
...
#         """Create comprehensive visualization including class imbalance effects"""
#         save_dir = Path(save_dir)
        
#         # 1. Per-class performance with support visualization
#         fig_performance = make_subplots(
#             rows=2, cols=2,
#             subplot_titles=('AP50 by Class', 'Class Distribution', 
#                           'AP50 vs Support', 'Precision-Recall by Class'),
...
#                 go.Bar(name=f'{split_name} AP50', x=classes, y=ap50_values),
#                 row=1, col=1
#             )
            
#             # Class distribution
#             fig_performance.add_trace(
#                 go.Bar(name=f'{split_name} Count', x=classes, y=supports),
#                 row=1, col=2
#             )
            
#             # AP50 vs Support (shows if performance correlates with data amount)
#             fig_performance.add_trace(
...
#             # Precision-Recall
#             fig_performance.add_trace(
#                 go.Bar(name=f'{split_name} Precision', x=classes, y=precisions),
#                 row=2, col=2
#             )
#             fig_performance.add_trace(
#                 go.Bar(name=f'{split_name} Recall', x=classes, y=recalls),
#                 row=2, col=2
#             )
        
#         fig_performance.update_layout(height=800, showlegend=True,
#                                      title_text="Comprehensive Performance Analysis")
#         fig_performance.update_xaxes(title_text="Support (# instances)", row=2, col=1)
#         fig_performance.update_yaxes(title_text="AP50", row=2, col=1)
        
#         fig_performance.write_html(str(save_dir / 'performance_analysis.html'))
        
#         # 2. Micro vs Macro comparison
...
#         return fig_performance, fig_comparison
...
#                 report.append(f"**Macro-averaged (treats all classes equally):**\n")
...
#                 # Interpretation
#                 diff = abs(metrics['macro_mAP50'] - metrics['micro_mAP50'])
#                 if diff > 0.1:
#                     report.append(f"⚠️ Large difference ({diff:.3f}) between macro and micro mAP50 indicates ")
#                     if metrics['micro_mAP50'] > metrics['macro_mAP50']:
#                         report.append("model performs better on common classes.\n\n")
#                     else:
#                         report.append("model performs better on rare classes.\n\n")
            
#             # Per-class metrics
#             if 'per_class' in split_results:
#                 report.append("### Per-Class Performance:\n")
#                 report.append("| Class | AP50 | Support | Relative Freq | Note |\n")
#                 report.append("|-------|------|---------|---------------|------|\n")
...
#                     report.append(f"| {cls} | {metrics['AP50']:.3f} | {metrics['support']} | "
#                                 f"{metrics['relative_frequency']:.1%} | {note} |\n")
...
#     parser.add_argument('--output', default='evaluation_results_enhanced', help='Output directory')
...
#             all_results[split] = evaluator.evaluate_on_split(split)
#     else:
#         all_results[args.split] = evaluator.evaluate_on_split(args.split)
...
#     evaluator.create_enhanced_plots(all_results, output_dir)
    
#     # Generate report
#     report = evaluator.generate_report(all_results, output_dir / 'evaluation_report.md')
#     print(report)

# scripts/evaluate_model_enhanced.py - PROPERLY FIXED
import json
from pathlib import Path
import numpy as np
import pandas as pd
...
class EnhancedModelEvaluator:
    ...
    def parse_training_logs(self):
        """Parse training CSV logs - FIXED PATH"""
        # The weights are at: models/yolov8n_20250908_225351/train/weights/best.pt
        # The results.csv is at: models/yolov8n_20250908_225351/train/results.csv
        # So from weights path, go up 2 levels to get to train folder
        
        results_csv = self.weights_path.parent.parent / 'results.csv'
        
        if results_csv.exists():
            logging.info(f"Found results.csv at {results_csv}")
    ...
    def create_training_plots(self, save_path=None):
        ...
        if df is None:
            logging.warning("Cannot create training plots - no results.csv found")
            return None
    ...
    def create_enhanced_plots(self, results, save_dir):
        """Additional plots for imbalance analysis"""
        save_dir = Path(save_dir)
        ...
        logging.info("AP50 vs support plot saved")
        ...
        fig_comparison.write_html(str(save_dir / 'micro_macro_comparison.html'))
        logging.info("Micro/macro comparison plot saved")
        
        return fig_support, fig_comparison
    
    def generate_report(self, results, save_path):
        """Generate detailed evaluation report - NO UNICODE CHARACTERS"""
        report = []
        ...
        for split_name, split_results in results.items():
            ...
            if 'overall_metrics' in split_results:
                ...
                report.append("**Macro-averaged (equal weight per class):**\n")
                report.append(f"  - mAP50: {metrics['macro_mAP50']:.3f}\n")
                report.append(f"  - mAP50-95: {metrics['macro_mAP50-95']:.3f}\n\n")
                report.append("**Micro-averaged (weighted by frequency):**\n")
                report.append(f"  - mAP50: {metrics['micro_mAP50']:.3f}\n")
                report.append(f"  - mAP50-95: {metrics['micro_mAP50-95']:.3f}\n\n")
            ...
            if 'per_class' in split_results:
                ...
                for cls, metrics in split_results['per_class'].items():
                    ...
                    if metrics['support'] < 50:
                        ...
                        note = "[X] Poor performance"
                    elif metrics['AP50'] > 0.7:
                        note = "[OK] Good performance"
